 ...
 * @version 0.1.0
 */
class TCPMockWorkerSpec extends MockItSpec {

    import TCPMockWorkerSpec._

    "A TCPMockWorker" should "run a MockUnit without a fault simulation" in {
        TCPMockWorkerSpec.runTest(new TestTCPMock, NoFault()) should be (false)
    }

    it should "run a MockUnit with delay simulation" in {
        TCPMockWorkerSpec.runTest(new TestTCPMockFixedDelay, FixedDelay(delay)) should be (false)
    }

    it should "run a MockUnit with connection loss simulation" in {
        TCPMockWorkerSpec.runTest(new TestTCPMockLooseConnection, LooseConnection()) should be (false)
    }

    it should "run a MockUnit with response multiplication simulation" in {
        TCPMockWorkerSpec.runTest(new TestTCPMockMultipleResponses, MultipleResponses(factor), responseNum = factor) should be (false)
    }
